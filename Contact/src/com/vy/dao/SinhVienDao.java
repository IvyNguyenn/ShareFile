...
public static int add(SinhVien sv){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into sinhvien(mssv,name,password,email,sdt) values(?,?,?,?,?)");
...
public static int update(SinhVien sv){
	...
	try{
		Connection con=getConnection();
		PreparedStatement ps=con.prepareStatement("update sinhvien set mssv=?,name=?,password=?,email=?,sdt=? where ID=?;");
		ps.setString(1,sv.getMssv());
		ps.setString(2,sv.getName());
		ps.setString(3,MD5Password.MD5(sv.getPassword()));
		ps.setString(4,sv.getEmail());
